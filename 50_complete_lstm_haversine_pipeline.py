"""
Complete Enhanced LSTM Pipeline with Haversine Distance & Hyperparameter Tuning

This is a consolidated version that includes all functionality in one file.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/complete_lstm_haversine.log', encoding='utf-8'),
...
# Setup MLflow
mlflow.set_experiment("Complete_LSTM_Haversine_Tuning")
...
def calculate_haversine_errors(y_true, y_pred):
    """Calculate haversine distance errors in meters."""
    lat_true = y_true[:, 0]
    lon_true = y_true[:, 1]
    lat_pred = y_pred[:, 0]
    lon_pred = y_pred[:, 1]
...
class EnhancedLSTMModel(nn.Module):
    ...
    def __init__(self, input_size, hidden_size=128, num_layers=4, output_size=4, dropout=0.1, bidirectional=False):
...
def perform_eda(df, output_dir='results/complete_lstm_haversine'):
