"""
Enhanced Pipeline: EDA → Clustering → PCA → LSTM Training with Early Stopping
- Comprehensive EDA with feature analysis
- Clustering and PCA for feature engineering
- Increased model complexity
- Early stopping with patience
- Training curves plotted per epoch
- Predictions for 30 random vessels
"""
...
class EnhancedLSTMModel(nn.Module):
    """Enhanced LSTM with increased complexity."""
    
    def __init__(self, input_size, hidden_size=128, num_layers=2, output_size=4, dropout=0.3):
...
def perform_eda(df):
    ...
    plt.tight_layout()
    plt.savefig('02_eda_correlation.png', dpi=300, bbox_inches='tight')
    logger.info("✓ Saved: 02_eda_correlation.png")
    plt.close()
...
def apply_clustering_and_pca(df, features, n_clusters=5, n_components=10):
    ...
    logger.info("✓ Saved: 03_pca_variance.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('04_clusters_map.png', dpi=300, bbox_inches='tight')
    logger.info("✓ Saved: 04_clusters_map.png")
    plt.close()
...
def train_model_with_early_stopping(X_train, y_train, X_val, y_val, epochs=200, batch_size=32, patience=20):
    """Train LSTM with early stopping and plot training curves."""
    logger.info(f"\n{'='*70}\n[6/10] TRAINING LSTM WITH EARLY STOPPING\n{'='*70}")
    
    mlflow.set_experiment("Enhanced_LSTM_AIS_Pipeline")
    ...
    with mlflow.start_run():
        ...
        model = EnhancedLSTMModel(input_size=X_train.shape[2], hidden_size=128, num_layers=2, dropout=0.3).to(device)
        ...
        mlflow.log_params({
            ...
            'patience': patience, 'train_samples': len(X_train), 'val_samples': len(X_val)
        })
        ...
        for epoch in tqdm(range(epochs), desc="Epochs"):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                torch.save(model.state_dict(), 'best_lstm_model_enhanced.pt')
            else:
                patience_counter += 1
        ...
        mlflow.pytorch.log_model(model, "lstm_model_enhanced")
...
def plot_training_curves(train_losses, val_losses, train_maes, val_maes):
    ...
    # MAE curves
    axes[1].plot(epochs, train_maes, 'b-', linewidth=2, label='Training MAE', marker='o', markersize=4)
    axes[1].plot(epochs, val_maes, 'r-', linewidth=2, label='Validation MAE', marker='s', markersize=4)
    ...
    plt.tight_layout()
    plt.savefig('05_training_curves.png', dpi=300, bbox_inches='tight')
    logger.info("✓ Saved: 05_training_curves.png")
    plt.close()
...
def evaluate_and_visualize(model, X_test, y_test, df, features, scaler, device):
    ...
    logger.info(f"\n{'='*70}\n[9/10] PLOTTING PREDICTIONS (30 Random Vessels)\n{'='*70}")
    ...
    plt.tight_layout()
    plt.savefig('06_predictions_30_vessels.png', dpi=300, bbox_inches='tight')
    logger.info("✓ Saved: 06_predictions_30_vessels.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('07_timeseries_predictions.png', dpi=300, bbox_inches='tight')
    logger.info("✓ Saved: 07_timeseries_predictions.png")
    plt.close()
...
if __name__ == "__main__":
    logger.info("\n" + "="*70)
    logger.info("ENHANCED PIPELINE: EDA → CLUSTERING → PCA → LSTM → EVALUATION")
    logger.info("="*70)
    ...
    df, X_pca, pca, kmeans = apply_clustering_and_pca(df, features, n_clusters=5, n_components=10)
    ...
    model, train_losses, val_losses, train_maes, val_maes, device = train_model_with_early_stopping(
        X_train, y_train, X_val, y_val, epochs=200, batch_size=32, patience=20
    )
